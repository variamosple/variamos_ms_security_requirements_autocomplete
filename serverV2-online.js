...
function elementInSentence(obj,sentence){
    ...
    Object.keys(obj).some(element => {
        if(sentence.includes(element)) {console.log(element); str=element;}
    })
    return str;
}
...
app.post('/suggest',async function(req, res, next) {
  console.log(req.body.input.trim()+"Test")
  res.setHeader('Content-Type', 'application/json');
  
  var words=(req.body.input.trimEnd()).split(" ");
 //console.log(words[words.length-1]);
  //if(words[words.length-1]=='') console.log(words.pop());
 ...
 // if(req.body.q=="")res.end(JSON.stringify(Object.keys(conditions1)));
  ...
  else if(a1 && !sc && !a2 && !sm && !vc && !ad && !c2 && !aa)
  {
       ...
       !activity[elementInSentence(activity,sentence)]["next"].includes((words[words.length-1]))) 
            if(Array.isArray(activity[elementInSentence(activity,sentence)]["next"]))options.push.apply(options,activity[elementInSentence(activity,sentence)]["next"]);
